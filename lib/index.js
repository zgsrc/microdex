...
exports.index = function(domain, macro, micros, cb) {
    if (micros && micros.length) {
        ...
        async.parallel([
            function(cb) { schema.tables.domains.addDocuments(domain, macro, micros, cb); },
            function(cb) { schema.tables.macros.addDocuments(domain, macro, micros, cb); },
            function(cb) { schema.tables.terms.addDocuments(domain, macro, micros, cb); }
        ], cb);
    }
    else cb();
};
...
exports.delete = function(domain, macro, micros, cb) {
    if (micros && micros.length) {
        micros = micros.compact(true).unique();
        async.parallel([
            cb => { schema.tables.macros.removeDocuments(domain, macro, micros, cb); },
            cb => { schema.tables.terms.removeDocuments(domain, macro, micros, cb); }
        ], cb);
    }
    else cb();
};
